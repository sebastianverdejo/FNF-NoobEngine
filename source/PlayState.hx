...
class PlayState extends MusicBeatState
{
	...
	public var cameraSpeed:Float = 2.5;
	...
	override public function create()
	{
		...
		timeTxt = new FlxText(STRUM_X + (FlxG.width / 2) - 248, 5, 400, "", 32);
		timeTxt.setFormat(Paths.font("vcr.ttf"), 41, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		if(ClientPrefs.scoreTxtPos == 'Right')
		{
			scoreTxt.x = healthBarBG.x - 144;
		}

		if(ClientPrefs.scoreTxtPos == 'Left')
		{
			scoreTxt.x = healthBarBG.x - 544;
		}

		if(ClientPrefs.scoreTxtPos == 'Disabled')
		{
			scoreTxt.y = healthBarBG.y + 324376345;
		}

		var leveli:FlxText = new FlxText(0, 680, 0, "", 20);
		leveli.text += PlayState.SONG.song;
		leveli.scrollFactor.set();
		leveli.borderSize = 1.25;
		leveli.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		leveli.updateHitbox();

		if (ClientPrefs.watermarkLayout == 'Side by Side')
		{
			scoreTxt.scrollFactor.set();
		}
		else
		{
			if (ClientPrefs.hideWatermark == true)
			{
				scoreTxt.scrollFactor.set();
			}
			else
		...
		var player:FlxText = new FlxText(0, 500, 0, "", 20);
		player.text += boyfriend.curCharacter;
		player.scrollFactor.set();
		player.borderSize = 1.25;
		player.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		player.updateHitbox();
		// add(player);

		var leveld:FlxText = new FlxText(0, 660, 0, "", 20);
		leveld.text += CoolUtil.difficultyString();
		leveld.scrollFactor.set();
		leveld.borderSize = 1.25;
		leveld.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		leveld.updateHitbox();
		...
		var watermark:FlxText = new FlxText(0, 700, 0, "", 20);

		if (ClientPrefs.watermarkLayout == 'Side by Side')
		{
			watermark.text += PlayState.SONG.song + " " + CoolUtil.difficultyString() + " | Noob Engine " + MainMenuState.psychEngineVersion;
		}
		else
		{
			watermark.text += "Noob Engine " + MainMenuState.psychEngineVersion;
		}
		...
		var timetxtLeft:FlxText = new FlxText(248, 19, 400, "", 20);
		timetxtLeft.setFormat(Paths.font("vcr.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		timetxtLeft.scrollFactor.set();
		timetxtLeft.borderSize = 1.25;
		// add(timetxtLeft);

		function startSong()
		{
			timetxtLeft.text = "/ " + FlxStringUtil.formatTime(Math.max(0, Math.floor(songLength / 1000)), false);
		}
		...
		leveli.cameras = [camHUD];
		leveld.cameras = [camHUD];
		player.cameras = [camHUD];
		timetxtLeft.cameras = [camHUD];
  ...
  public function clearShaderFromCamera(cam:String){
	...
	override public function update(elapsed:Float)
	{
		...
		if(ratingName == '') {
			scoreTxt.text = 'Score: ' + songScore + ' | Combo Breaks: ' + songMisses + ' | Accuracy: ' + '0%';
		} else {
			scoreTxt.text = 'Score: ' + songScore + ' | Combo Breaks: ' + songMisses + ' | Health: ' + healthBar.percent + ' | Accuracy: ' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%' + ' | ' + ratingFC;//peeps wanted no integer rating
		}
		...
		if (ClientPrefs.Iconbop == 'Original')
		{
		...
		{
			iconP1.setGraphicSize(Std.int(FlxMath.lerp(150, iconP1.height, 0.8)),Std.int(FlxMath.lerp(150, iconP1.height, 0.8)));
		}
		...
		else
		{
			...
			if (!paused)
			{
				...
				if(updateTime) {
					...
					if(ClientPrefs.timeBarType != 'Song Name')
						timeTxt.text = FlxStringUtil.formatTime(secondsTotal, false) + ' / ' + FlxStringUtil.formatTime(Math.max(0, Math.floor(songLength / 1000)), false);
	...
	override function beatHit()
	{
		...
		{
			iconP1.setGraphicSize(Std.int(iconP1.height + (55 * (1))),Std.int(iconP1.height - (25 * (1))));
			iconP2.setGraphicSize(Std.int(iconP2.height + (50 * (1))),Std.int(iconP2.height - (25 * (1))));
		...
		{
			iconP1.setGraphicSize(Std.int(iconP1.width + (55 * (1))),Std.int(iconP1.height - (25 * (1))));
			iconP2.setGraphicSize(Std.int(iconP2.width + (50 * (1))),Std.int(iconP2.height - (25 * (1))));
		}
